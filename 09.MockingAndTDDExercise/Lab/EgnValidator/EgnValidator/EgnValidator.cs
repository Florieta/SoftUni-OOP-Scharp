...
namespace EgnValidatorProgram
{
    ...
    public class EgnValidator : IEgnValidator
    {
        ...
        public EgnInfo GetEgnInfo(string egn)
        {
            if (!this.Validate(egn))
            {
                throw new ArgumentException("Invalid EGN provided.", "egn");
            }

            var egnInfo = new EgnInfo();
            if (int.Parse(egn[8].ToString()) % 2 == 0)
            {
                egnInfo.Gender = Gender.Male;
            }
            else
            {
                egnInfo.Gender = Gender.Female;
            }

            var year = int.Parse($"{egn[0]}{egn[1]}");
            int month = int.Parse($"{egn[2]}{egn[3]}");
            int day = int.Parse($"{egn[4]}{egn[5]}");
            if (month > 40)
            {
                year = 2000 + year;
                month -= 40;
            ...
            egnInfo.BirthDate = new DateTime(year, month, day);

            int regionId = int.Parse($"{egn[6]}{egn[7]}{egn[8]}");
            var region = this.Regions.FirstOrDefault(x => x.Value.Contains(regionId));
            egnInfo.City = region.Key;
